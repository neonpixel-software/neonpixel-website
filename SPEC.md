...
- Visiting `/` redirects (or otherwise routes) to `/en/`, and the Home/404 pages render correctly under that `/en/` prefix.
...
19. **Resolved — full render proven end-to-end, but with a serious open item for production.** With a real Home document type, a published content node, and its template explicitly assigned, the Home page renders correctly through `theme/Views/HomeContent.cshtml` and `theme/Views/Shared/_Layout.cshtml`, static assets included. Two real bugs were found and fixed along the way, kept here for the record:
    ...
    - **This is a real, unresolved gap for Phase 4 (Release & Deploy).** `DEPLOYMENT.md`'s systemd unit sets `ASPNETCORE_ENVIRONMENT=Production` (correct, standard practice) — under the current setup, that would very likely reproduce the exact "No physical template file was found" failure in production. Before Task 15 (cut release, ship to production), this needs a real decision: either (a) switch off `InMemoryAuto` to a source-code-generated ModelsBuilder mode with `RazorCompileOnBuild=true`, moving `theme/Views` to an MSBuild-level Razor compile-include instead of a runtime file provider, or (b) some other supported way to get runtime compilation active outside `IsDevelopment()`. Do not attempt to "fix" this by simply setting `ASPNETCORE_ENVIRONMENT=Development` in production — that also disables other production-appropriate behavior (detailed error pages, etc.) and is not a real fix.
