...
19. Both the public repo's CI (build+test) and the deploy workflow need read access to the private `neonpixel-theme` repo to check out the submodule — via a dedicated, read-only deploy key stored as a GitHub secret (e.g. `THEME_REPO_DEPLOY_KEY`), separate from the VPS deploy key. Local developers authenticate with their own GitHub credentials/SSH key when running `git submodule update --init`.
...
.github/workflows/ci.yml     → Build+test on push to develop and PRs into develop/main (checks out theme/ via a read-only deploy key)
...
## CI/CD
GitHub Actions, two triggers on the same build/test job, one additional deploy job. Every job checks out this repo **and** the `theme/` submodule (`actions/checkout` with `submodules: true` and an `ssh-key` pointed at a read-only `THEME_REPO_DEPLOY_KEY` secret scoped to the private `neonpixel-theme` repo — separate from the VPS deploy key):
...
17. Where does the private `neonpixel-theme` repo get created (which GitHub account/org), and who besides the primary developer needs read access to it (e.g. other contributors, the GitHub Actions deploy key)?
...
19. Is a Razor-view-location + static-file-provider addition in `Program.cs` (pointing at `theme/Views` and `theme/wwwroot`) actually sufficient, or does Umbraco 18's view/asset pipeline need something more specific (e.g. its own convention for where front-end templates live) — needs confirming against Umbraco 18 docs at Plan time, same caveat as Open Question 1.
