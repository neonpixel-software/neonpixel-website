...
20. **New requirement: the site must be multilingual.** Only English ships with real content at launch, but the *architecture* (culture-variant content properties, language-prefixed URL routing) goes in now so additional languages are a content-only fast-follow later, not a re-platforming. URL strategy is subfolder-per-language (e.g. `/en/`, `/nl/`), not domain-per-language. To avoid a breaking URL change when a second language is added later, English gets the `/en/` prefix from day one rather than living at bare `/` — this is a deliberate choice, not something Umbraco does automatically for a single-language site (confirm this still matches intent once a second language is actually added). The purchased template has no built-in language-switcher UI; one needs to be designed to match the site's existing header (near the "Say Hello" button / color-switcher toggle), shown even with just one language listed so it doesn't need retrofitting later.
...
- A language switcher is visible in the header, matching the template's visual style, even though only one language exists at launch.
...
23. What should the language switcher actually look like/say with only one language present ("EN" with a dropdown that has one item? Hidden until a second language exists? A visible-but-disabled control)? Needs a design decision before building it, not just an engineering one.
