...
**Dependencies:** Checkpoint: Template Integration; Task 5; Task 6 (executed, not just written); **new hard blocker confirmed during Task 12 testing** — the current `theme/` rendering approach only works with `ASPNETCORE_ENVIRONMENT=Development` set, and production runs as `Production`. This needs to be resolved (see the Risks table and SPEC.md Open Question 19) before this task can succeed — a straight `main` deploy today would very likely serve "No physical template file was found" errors instead of the real site.
...
| **Confirmed, not hypothetical:** the `theme/` file-provider approach only renders when `ASPNETCORE_ENVIRONMENT=Development` — `Umbraco.Cms.DevelopmentMode.Backoffice`'s runtime compilation appears gated on `IsDevelopment()`. Proven working end-to-end locally (real content node, real render); confirmed via SPEC.md Open Question 19 that production (`ASPNETCORE_ENVIRONMENT=Production`, as `DEPLOYMENT.md` sets) will very likely fail the same way | High — blocks Task 15 (ship to production) entirely until resolved | Needs a real decision before Task 15: either move off `InMemoryAuto` models mode to a source-code-generated mode with real build-time Razor compilation (theme views become an MSBuild compile-include instead of a runtime file provider), or find a supported non-Development way to activate runtime compilation. Do not "fix" by setting `ASPNETCORE_ENVIRONMENT=Development` in production. |
...
- **Confirm Umbraco 18's actual view/static-file extension mechanism** (SPEC.md Q19) — needed at Task 9, before Task 9 is considered done rather than just "working by trial."
