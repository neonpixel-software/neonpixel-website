...
- [ ] Task 15: Cut `release/1.0.0`, provision VPS, ship to production — no longer blocked on rendering or domain; still needs: deploy secrets set on GitHub (`VPS_HOST`, `VPS_USER`, `VPS_DEPLOY_KEY`, `VPS_KNOWN_HOSTS`, `VPS_DEPLOY_PATH`, `VPS_SERVICE_NAME`, `VPS_DB_PATH`, `VPS_ENV_FILE`, `VPS_SSH_PORT` — user is setting these themselves via the GitHub UI, not this session), DNS for `neonpixel.eu` pointed at the VPS (confirmed 2026-09-03: `neonpixel.eu` → `85.17.61.220`)
